...
class TestUserCrud():
    ...
    def test_user_list(self, client):
        ...
        res = client.get(
            '/user/admin', headers={'Authorization': 'Bearer ' + token})

        assert res.status_code == 200

    def test_user_list_invalid_token(self, client):
        res = client.get(
            '/user/admin', headers={'Authorization': 'Bearer abc'})
    ...
    def test_user_post(self, client):
        ...
        res = client.post('/user', data=json.dumps(inputan),
...
#
    ...
    def test_user_put(self, client):
        ...
        res = client.put('/user', data=json.dumps(inputan),
    ...
    def test_user_get_me(self, client):
        token = create_token_non()
        res = client.get('/user', content_type='application/json',
    ...
    def test_user_delete_invalid_admin(self, client):
        token = create_token_for_book()
        res = client.delete('/user/'+str(TestUserCrud.temp_client),
                            content_type='application/json', headers={'Authorization': 'Bearer ' + token})
